...
import { SchedulingUserPluginData } from '../../../../api/me/plugins';
import { PluginSchema } from '../../../../api/plugins';
import useUserPlugins from '../../../../hooks/useUserPlugins';

type EditSchedulingPluginFormProps = {
  data: [PluginSchema, SchedulingUserPluginData];
  onFinished?: () => void;
};

const EditSchedulingPluginForm = ({
  data: [schema, plugin],
  onFinished,
}: EditSchedulingPluginFormProps) => {
  const { create, update } = useUserPlugins();
  const { register, handleSubmit, control } = useForm<SchedulingUserPluginData>({
    defaultValues: {
      ...plugin,
    },
  });
  ...
  const handleFormSubmit = useCallback(
    async (data: SchedulingUserPluginData) => {
      try {
        const newValues = {
          ...plugin,
          ...data,
...
export default EditSchedulingPluginForm;
